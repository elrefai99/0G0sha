...
The `AgentEngine` class in `src/agent/script/agent-engine.ts` orchestrates the full 5-phase pipeline. `AgentService` in `Module/agent/Service/based-agent.service.ts` exposes it over HTTP.

Engine components in `src/agent/script/`:
- `agent-engine.ts` — Main orchestrator: `init()` seeds weights, `process()` runs all 5 phases
- `tokenizer.ts` — Text → weighted keyword tokens
- `classifier.ts` — Category classification + complexity assessment
...
- `learner.ts` — `Learner` class: READ (`findSimilar`, `getWeights`), WRITE (`recordResult`), FEEDBACK (`applyFeedback`). `initWeights` seeds defaults (idempotent, called on startup)
- `merger.ts` — `Merger` class: borrows structure from high-scoring similar past prompts
- `modelAdapter.ts` — Format output for Claude (XML), GPT (markdown), or general
- `weight-cache.ts` — In-memory weight cache with TTL
...
| Frontend | Vue 3 + Vite + UnoCSS (in `frontend/` workspace package) |

### Monorepo Structure

PNPM workspace with two packages:
- `.` (root) — Backend (Express + Node.js)
- `frontend/` — Vue 3 SPA (`@0gosha/frontend`), builds with Vite, deployed to Netlify

Frontend dev: `cd frontend && pnpm dev` (port 5173).
...
`src/gen-import.ts` is the **single barrel file** for all cross-module imports. Never import directly across module boundaries — always import from `@/gen-import`. It is auto-generated by `pnpm gen:app`. Run that command after adding any new export anywhere in `src/`.
...
### Mounted Routes (`app.module.ts`)

- `/api/v1/auth` — Authentication (register, login, refresh, logout, Google OAuth, forget/reset password)
- `/api/v1/users` — User profile (requires `profileMiddleware`)
- `/api/v1/agent` — Agent analysis (`POST /analyze`)
- `/api/v1/notifications` — Real-time notifications via SSE backed by Redis pub/sub
- `/api/v1/prompts` — Prompt history and optimization
- `/api/v1/templates` — Prompt templates
- `/api/v1/subscriptions` — Subscription/plan management
- `/api/webhooks` — Payment webhook handlers (Stripe/Paymob)
...
- Import across modules only via `@/gen-import`; run `pnpm gen:app` after any new export.
